...
export async function POST(request: Request) {
  ...
  if (!supabase) {
    return NextResponse.json(
      { error: 'Supabase credentials are missing. Add them in .env.local or Vercel.' },
      { status: 503 },
    );
  }
  ...
  // License number and GST are now optional
  ...
  const gstDoc = formData.get('gst_doc') as File | null;

  if (!fullName || !isValidPhone(phone) || !isValidPhone(whatsapp) || !yearsExperience) {
  ...
  if (licenseDoc && licenseDoc.size > maxDocBytes) {
    return NextResponse.json({ error: 'License document must be 5MB or less.' }, { status: 400 });
  }
  if (gstDoc && gstDoc.size > maxDocBytes) {
    return NextResponse.json({ error: 'GST document must be 5MB or less.' }, { status: 400 });
  }

  try {
    const profilePhotoUrl =
      profilePhoto && profilePhoto.size > 0 ? await uploadFile('profile-photos', phone, profilePhoto) : null;
    const licenseDocUrl =
      licenseDoc && licenseDoc.size > 0 ? await uploadFile('license-docs', phone, licenseDoc) : null;
    const gstDocUrl =
      gstDoc && gstDoc.size > 0 ? await uploadFile('license-docs', `${phone}-gst`, gstDoc) : null;
    ...
    const adminWhatsappUrl = adminPhone
      ? `https://wa.me/${adminPhone}?text=${encodeURIComponent(
          `New Booksurveyor.co registration: ${fullName}, phone ${phone}${licenseNumber ? `, license ${licenseNumber}` : ''}${gstNumber ? `, GST ${gstNumber}` : ''}`,
        )}`
      : null;
